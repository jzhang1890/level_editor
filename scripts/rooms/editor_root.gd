...
@onready var z_order_spinbox: SpinBox = $EditorUI/ColorChannelMenu/ZOrderSpinBox

@onready var color_picker_btn: ColorPickerButton = $EditorUI/ColorChannelMenu/ColorPickerButton
@onready var fade_time_input = $EditorUI/ColorTriggerMenu/FadeTimeContainer/FadeTimeInput
...
func _ready() -> void:
	...
	$EditorUI/ColorChannelMenu/ChannelBGButton.pressed.connect(_on_color_channel_selected.bind(-1))
	
	# Bind Z-Layer buttons to their integer values
	var z_layer_menu = $EditorUI/ColorChannelMenu/ZLayerContainer
